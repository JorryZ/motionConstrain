...
History:
    ...
    ---------- ---------- ----------------------------
  Author: jorry.zhengyu@gmail.com         01MARCH2019           -V1.0 Created,velocityWise
  Author: jorry.zhengyu@gmail.com         01APRIL2019           -V2.0 Add displacementWise
  Author: jorry.zhengyu@gmail.com         04MAY2019             -V2.1 Add Weight
  Author: jorry.zhengyu@gmail.com         12JUNE2019             -V2.2 Add samplePoints function
  Author: jorry.zhengyu@gmail.com         12JUNE2019             -V2.3 Add errorCalc function
  Author: jorry.zhengyu@gmail.com         12JUNE2019             -V2.4 Add normalization weight in solve_displacementWise, UNDO
...
class motionConstrain:
    ...
    def samplePointsFromFile(self,meshFilePath=None,scale=1.,spacingDivision=[5.,1.5],bgGlobalField=False,skip=None):
        '''
        ...
        '''
        if meshFilePath[-3:]=='stl':
            ref_mesh=trimesh.load(meshFilePath)
            oriPos=np.array(ref_mesh.vertices)/scale
            if type(skip)!=type(None):
                temp=[]
                for i in range(len(oriPos)):
                    if i%skip==0:
                        temp.append(oriPos[i])
            else:
                temp=oriPos
            self.sampleCoord=np.array(temp.copy())
            print('sample coordinates from STL file,  have ',len(self.sampleCoord),' points')

        elif meshFilePath[-1][-3:]=='vtk':
            rawImage=medImgProc.load(meshFilePath[0])
            truthData=medImgProc.imread(meshFilePath[1])
            truthData.dim=['z','y','x']
            truthData.dimlen={}
            for dims in truthData.dim:
                truthData.dimlen[dims]=rawImage.dimlen[dims]/scale
            truthData.rearrangeDim(['x','y','z'])
            print('Dimlen: ',truthData.dimlen)
            self.dimlen=truthData.dimlen
            del rawImage
            
            samplepts1=self.bsFourier.samplePoints(spacingDivision=spacingDivision[0])
            wall=[]
            for m in range(len(samplepts1)):
                if truthData.getData(samplepts1[m])>=0.5:
                    wall.append(samplepts1[m].copy())
            print('incompressible',len(wall),'points')
            ...
            if bgGlobalField==False:
                ...
                for m in range(len(samplepts2)):
                    if truthData.getData(samplepts2[m])<0.5:
                        bg.append(samplepts2[m].copy())
                print('compressible',len(bg),'points')
    ...
    def solve_velocityWise(self,sampleCoord=None,bgCoord=None,maxError=0.0001,maxIteration=1000,fterm_start=1,convergence=0.8,reportevery=1000,saveFtermPath=None,tempSave=None,resume=False,movAvgError=False,lmLambda_init=0.001,lmLambda_incrRatio=5.,lmLambda_max=float('inf'),lmLambda_min=0.):
        if sampleCoord==None:
            sampleCoord=self.sampleCoord
        if bgCoord==None:
            bgCoord=self.bgCoord
        sampleCoordSize=len(sampleCoord)
        bgCoordSize=len(bgCoord)
        print('sampleCoord has %d points, bgCoord has %d points.'%(sampleCoordSize,bgCoordSize))
        #wdiag=np.ones(sampleCoordSize)
        n=np.prod(self.coefMat.shape[:3])    #number of control points
        ft=self.coefMat.shape[3]
        uvw=self.coefMat.shape[4]
